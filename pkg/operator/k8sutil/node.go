...
func GetNodeSchedulable(node v1.Node) bool {
	...
	// 2. Then k8s node controller will update node a second time to remove the
	//    node.kubernetes.io/unschedulable taint.
	//    Rook will discard this event because node.Spec.Unschedulable hasn't changed since previous update
	//    => Uncordon node isn't processed by rook and not reintegrated into cluster upon restart of the operator.
	//
